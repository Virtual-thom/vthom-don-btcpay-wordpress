<?php
/*
Plugin Name: BTCPay Server Donors
Description: Automatically register a donor when a BTCPay Server invoice is paid (a minimum amount can be set). Donors can also be added manually. Display donors list with a WP shortcode.
Author: Lounès Ksouri, Virtual Thom
*/
...
function btcpay_donors_form_add_donor($request)
{
  if (isset($_POST["add_donor_meta_nonce"]) && wp_verify_nonce($_POST["add_donor_meta_nonce"], "add_donor_meta_form_nonce")) {
    ...
    $donor = btcpay_donors_add_donor($donor_name);
